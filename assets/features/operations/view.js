...
function operationModal(m){const p=pr(m.id),kind=m.op||m.operation;m.op=kind;if(!p||!Object.hasOwn(opLabels,kind)){closeModal();return}const scope=resourcesFor(p,kind);let body=`<div class="stack">${detail([['项目',h(p.name)],['服务器',h(sname(p.server))],['当前状态',lifeName[p.life]+' · '+runtimeName[p.runtime]],['操作',opLabels[kind]],['本次资源范围',`<code>${h(scope.join('\n'))}</code>`]])}`;
