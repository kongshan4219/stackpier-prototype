...
function stateGrid(p){const op=activeOps(p)[0]||S.operations.find(o=>o.project===p.id);return `<div class="state-grid"><div class="state-cell"><small>部署阶段</small><strong>${lifeName[p.life]}</strong><span class="sub">${p.components.length?'存在未完成的变更':'核心资源的管理阶段'}</span></div><div class="state-cell"><small>最近实际观测</small>${runtimeBadge(p)}<span class="sub">${fmt(p.observed)}${p.runtimeCheckStatus==='unknown'?' · 保留旧观测':''}</span></div><div class="state-cell"><small>巡检运行基线</small><strong>${p.life==='uninstalled'||p.life==='draft'?'—':p.desired==='running'?'已核实运行':p.stopVerified?'主动停止已核实':'停止基线待核对'}</strong><span class="sub">本次请求只记入操作，不改写实际状态</span></div><div class="state-cell"><small>保存与应用</small><strong>${pending(p)?'有待应用内容':'已应用'}</strong><span class="sub">草稿 D${p.draftRev} · 参照 ${p.applied?'A'+p.appliedRev:'未建立'}</span></div><div class="state-cell"><small>${activeOps(p).length?'当前操作':'最近操作'}</small>${op?status(op.status):'<strong>暂无操作</strong>'}<span class="sub">${h(op?.label||'保存不等于远端执行')}</span></div></div>`}
