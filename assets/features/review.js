...
registerPrototypeHandlers(prototypeModals, ["about","feedback","reviewsettings","scenes","resetconfirm"], function(m, p) {
  switch (m.kind) {
case'about':layout('原型说明','从现有原型和前端延续，而不是另起一套视觉体系。',`<div class="stack">${notice('这是可交互的模拟应用','所有数据都在当前浏览器；没有真实 SSH、数据库、通知、DNS、防火墙或定时任务。请勿输入真实凭据。')}<div class="detail-list">${detail([['参考提交','<code>ef41df2bd4001ddee61fb60203e9fa74aebb7d52</code>'],['已读取原型','source/main.jsx、source/styles.css'],['已读取前端','WorkspaceShell.tsx、ServerFields.tsx、servers.css'],['现行需求','主仓库 docs/prototype-requirements.md'],['暂缓 / 不做','备份与恢复、复制迁移、独立回退、网页终端、通用文件管理']])}</div><p class="small muted">样式、品牌和服务器紧凑布局沿用仓库。业务范围来自需求基线；新增页面结构和状态行为仍待审。此 HTML 不是正式 React 前端实现，也不固定后端、API 或数据库设计。</p>${notice('纯静态交互原型','此文件通过 GitHub Pages 托管，所有操作仍仅在当前浏览器内模拟。网页可以公开访问，演示登录不是实际访问保护。')}<p class="small muted">刷新保留非敏感演示状态；在途模拟操作会变为待核对，不自动重放。浏览器存储被清理后会恢复样例。页面标注仅存在此浏览器，不会自动发送给对话。</p></div>`,btn('重置全部演示','resetconfirm',{},'danger')+btn('查看试用设置','reviewsettings')+btn('开始体验','closemodal',{},'primary'),true);break;
...
case'reviewsettings':S.review.p1=get('review-p1');persist();closeModal();toast('已更改演示分支；不代表任何产品决定已获批准。');break;
  }
});
